...
class Depot: PlayerBuilding {
    ...
    func getBuildingWithHighestNeed(gameState: GameState) -> PlayerBuilding? {
        ...
        // Exclude depots - they're handled separately by getDepotWithHighestNeed
        let nonDepots = inRange.filter { !($0 is Depot) }
        return nonDepots.filter { $0.needsAnyResource }.max { $0.totalDeficit < $1.totalDeficit }
    }
...
class SniperTower: PlayerBuilding {
    ...
    private func handleExcessInventory(gameState: GameState) {
        ...
        // Only send excess bullets if we have more than quota AND surplus survivors
        if presentBullets >= (bulletQuota + 10) && !hasZombiesInRange {
            if presentSurvivors > survivorQuota {  // Must have surplus survivors to send
    ...
    private func sendBulletsToDepot(gameState: GameState, targetDepot: Depot) {
        guard presentSurvivors > survivorQuota else { return }  // Only send if we have surplus survivors
        ...
        truck.isFromWorkshop = true  // Treat like workshop - survivor stays at depot
