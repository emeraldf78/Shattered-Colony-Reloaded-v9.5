...
class GameScene: SKScene {
    ...
    private func createQuotaRow(value: Int, xPos: CGFloat, yPos: CGFloat, tag: String) -> SKNode {
        ...
        // Value box
        let valueBox = SKShapeNode(rectOf: CGSize(width: 28, height: 20), cornerRadius: 3)
    ...
    private func createDepotMenu(for depot: Depot) -> SKNode {
        let panelWidth: CGFloat = 210
        let panelHeight: CGFloat = 175
        
        let panel = createMenuPanel(width: panelWidth, height: panelHeight)
        panel.addChild(createWoodenHeader(title: "DEPOT", width: panelWidth, yPos: panelHeight/2 - 18))
        
        // Current inventory row
        let invY: CGFloat = panelHeight/2 - 55
        panel.addChild(createResourceDisplay(emoji: "🔫", value: depot.storedBullets, xPos: -55, yPos: invY, name: "inv_bullets"))
        panel.addChild(createResourceDisplay(emoji: "🪵", value: depot.storedWood, xPos: 0, yPos: invY, name: "inv_wood"))
        panel.addChild(createResourceDisplay(emoji: "👤", value: depot.heldSurvivors, xPos: 55, yPos: invY, name: "inv_survivors"))
        
        // Quota row
        let quotaY: CGFloat = invY - 50
        panel.addChild(createQuotaRow(value: depot.bulletQuota, xPos: -55, yPos: quotaY, tag: "bullets"))
        panel.addChild(createQuotaRow(value: depot.woodQuota, xPos: 0, yPos: quotaY, tag: "wood"))
        panel.addChild(createQuotaRow(value: depot.survivorQuota, xPos: 55, yPos: quotaY, tag: "survivors"))
        
        // Quota label
        panel.addChild(createQuotaLabel(yPos: quotaY - 22))
        
        // Action buttons on right
        let btnX: CGFloat = panelWidth/2 - 28
        ...
        panel.addChild(createActionButton(icon: "🏃", color: SKColor(red: 0.7, green: 0.3, blue: 0.3, alpha: 1.0), xPos: btnX, yPos: invY - 42, name: "retreat"))
    ...
    private func createSniperMenu(for sniper: SniperTower) -> SKNode {
        ...
        let panelHeight: CGFloat = 185
        
        let panel = createMenuPanel(width: panelWidth, height: panelHeight)
        panel.addChild(createWoodenHeader(title: "SNIPER", width: panelWidth, yPos: panelHeight/2 - 18))
        
        // Accuracy bar
        let accY: CGFloat = panelHeight/2 - 50
        let accBarBg = SKShapeNode(rectOf: CGSize(width: 120, height: 18), cornerRadius: 3)
        accBarBg.fillColor = SKColor(red: 0.3, green: 0.3, blue: 0.35, alpha: 1.0)
        accBarBg.strokeColor = .gray
        accBarBg.lineWidth = 1
        accBarBg.position = CGPoint(x: -15, y: accY)
        panel.addChild(accBarBg)
        
        let accBarFill = SKShapeNode(rectOf: CGSize(width: max(2, 116 * sniper.accuracy), height: 14), cornerRadius: 2)
        accBarFill.fillColor = SKColor(red: 0.3, green: 0.7, blue: 0.3, alpha: 1.0)
        accBarFill.strokeColor = .clear
        accBarFill.position = CGPoint(x: -15 - 58 + (116 * sniper.accuracy)/2, y: accY)
        ...
        accLabel.position = CGPoint(x: -15, y: accY - 1)
        accLabel.verticalAlignmentMode = .center
        panel.addChild(accLabel)
        
        // Current inventory row
        let invY: CGFloat = accY - 35
        panel.addChild(createResourceDisplay(emoji: "👤", value: sniper.presentSurvivors, xPos: -35, yPos: invY, name: "inv_survivors"))
        panel.addChild(createResourceDisplay(emoji: "🔫", value: sniper.presentBullets, xPos: 25, yPos: invY, name: "inv_bullets"))
        
        // Quota row
        let quotaY: CGFloat = invY - 50
        panel.addChild(createQuotaRow(value: sniper.survivorQuota, xPos: -35, yPos: quotaY, tag: "survivors"))
        panel.addChild(createQuotaRow(value: sniper.bulletQuota, xPos: 25, yPos: quotaY, tag: "bullets"))
    ...
    private func createWorkshopMenu(for workshop: Workshop) -> SKNode {
        ...
        let panel = createMenuPanel(width: panelWidth, height: panelHeight)
        panel.addChild(createWoodenHeader(title: "WORKSHOP", width: panelWidth, yPos: panelHeight/2 - 18, icon: "⚙️"))
        
        // Collected resources (waiting to ship)
        let collectedY: CGFloat = panelHeight/2 - 55
        panel.addChild(createResourceDisplay(emoji: "🔫", value: workshop.collectedBullets, xPos: -50, yPos: collectedY, name: "collected_bullets"))
        panel.addChild(createResourceDisplay(emoji: "🪵", value: workshop.collectedWood, xPos: 0, yPos: collectedY, name: "collected_wood"))
        panel.addChild(createResourceDisplay(emoji: "👤", value: workshop.collectedSurvivors, xPos: 50, yPos: collectedY, name: "collected_survivors"))
        
        // "Left to Extract" label
        let extractLabelY: CGFloat = collectedY - 32
        panel.addChild(createSectionLabel(text: "LEFT TO EXTRACT", yPos: extractLabelY))
        
        // Resources left to extract
        let leftY: CGFloat = extractLabelY - 28
        var bulletsLeft = 0
        var woodLeft = 0
        var survivorsLeft = 0
        
        if let building = workshop.targetBuilding {
            bulletsLeft = building.resources.bullets
            woodLeft = building.resources.wood
            survivorsLeft = building.resources.survivors
        } else if let debris = workshop.targetDebris {
            bulletsLeft = debris.resources.bullets
            woodLeft = debris.resources.wood
            survivorsLeft = debris.resources.survivors
        ...
        leftSurvivors.fontSize = 12
        leftSurvivors.fontColor = .white
        leftSurvivors.position = CGPoint(x: 50, y: leftY)
        panel.addChild(leftSurvivors)
        
        // Quota row (just survivor quota for workshops)
        let quotaY: CGFloat = leftY - 30
        panel.addChild(createQuotaLabel(yPos: quotaY + 12))
        panel.addChild(createQuotaRow(value: workshop.survivorQuota, xPos: 0, yPos: quotaY - 10, tag: "survivors"))
        
        // Action button - just retreat (no blue button for workshop)
        let btnX: CGFloat = panelWidth/2 - 28
        panel.addChild(createActionButton(icon: "🏃", color: SKColor(red: 0.7, green: 0.3, blue: 0.3, alpha: 1.0), xPos: btnX, yPos: collectedY - 20, name: "retreat"))
    ...
    private func createBarricadeMenu(for barricade: Barricade) -> SKNode {
        ...
        let panel = createMenuPanel(width: panelWidth, height: panelHeight)
        panel.addChild(createWoodenHeader(title: "BARRICADE", width: panelWidth, yPos: panelHeight/2 - 18))
        ...
        // Retreat button
        let btnX: CGFloat = panelWidth/2 - 28
        panel.addChild(createActionButton(icon: "🏃", color: SKColor(red: 0.7, green: 0.3, blue: 0.3, alpha: 1.0), xPos: btnX, yPos: invY - 20, name: "retreat"))
